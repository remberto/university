...
d(document).ready(function(){   
   d('#dat_universidad').change(function(){
       d.ajax({
            type: 'POST',
            url:  '<?php echo url_for('registro/sedes'); ?>',
            data: 'universidad_id='+d('#dat_universidad').val(),
            success: function(data){
                if(data == 0) {
                    //Query returned empty.
                } else {
                    //Query Has values.
                    d('#reg_inscripcion_uni_sede_id').html(data);
                }
            }
        });
    });
    d('#reg_inscripcion_uni_sede_id').change(function(){
       d.ajax({
            type: 'POST',
            url:  '<?php echo url_for('registro/gestionesinscripcion'); ?>',
            data: 'sede_id='+a('#reg_inscripcion_uni_sede_id').val()+'&carrera_id='+a('#reg_inscripcion_uni_carrera_id').val(),
            success: function(data){
                if(data == 0) {
                    //Query returned empty.
                } else {
